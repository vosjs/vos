...
| [`@vosso/vos-plugin`](./packages/vos-plugin)   | [![npm](https://img.shields.io/npm/v/%40vosso%2Fvos-plugin)](https://www.npmjs.com/package/@vosso/vos-plugin) | The take pipeline and the vos.so verbs the `vos` binary delegates to: record the real product, auto-zoom from the cursor track, cut as data, deliver a release's media per destination spec, sync with vos.so. Bundles the three libraries below. |
| [`@vosso/studio-core`](./packages/studio-core) | private, bundled                                                                                              | The studio's document model: the `ProjectDoc` schema, the element-aware auto-zoom planner, the lowering from a document to a vos composition, the timeline lane adapters.                                                                         |
| [`@vosso/render-core`](./packages/render-core) | private, bundled                                                                                              | The render harness: timeline-sharded chunk planning and stream-copy chunk concat for deterministic renders.                                                                                                                                       |
| [`@vosso/shared`](./packages/shared)           | private, bundled                                                                                              | The small shared layer under the plugin and the studio: the semantic differ, limits, frontmatter, params, the font and music catalogs.                                                                                                            |

> `three` and `gsap` are **optional peer dependencies** — you bring your own versions, and the engine never bundles them.

> The four `@vosso/*` packages are a **mirror** of vosso's monorepo, MIT like everything here; see [MIRROR.md](./MIRROR.md) for what that means for issues and pull requests.
