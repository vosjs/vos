...
describe('runCheck', () => {
  ...
  it('preserves params/presets through the pipeline without warnings', () => {
    const withParams = {
      ...VALID,
      params: [{ key: 'hue', label: 'Hue', kind: 'number', min: 0, max: 1, default: 0.5 }],
  ...
  it('warns on unknown top-level keys the platform would drop', () => {
    ...
    expect(r.ok).toBe(true)
    expect(r.issues.some((i) => i.source === 'shape' && /myExtra/.test(i.message))).toBe(true)
  })
  ...
  it('fails on a function string with a syntax error', () => {
    ...
    expect(r.ok).toBe(false)
    expect(r.issues.some((i) => i.source === 'syntax' && /createTimeline/.test(i.message))).toBe(
      true,
    )
  })
