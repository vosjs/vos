...
import { hasDeterminismErrors, lintVosConfig } from '../lint'
...
describe('lintVosConfig', () => {
  it('flags Math.random as an error', () => {
    const issues = lintVosConfig({
      ...base,
      onFrame: '(ctx) => { const r = Math.random() }',
    })
  ...
  it('flags gsap.utils.random', () => {
    const issues = lintVosConfig({
      ...base,
      createTimeline: '(ctx) => { ctx.gsap.utils.random(0, 1) }',
    })
  ...
  it("flags stagger from:'random'", () => {
    const issues = lintVosConfig({
      ...base,
      createTimeline:
        "(ctx) => { ctx.gsap.to(all, { y: 10, stagger: { from: 'random', each: 0.1 } }) }",
    })
  ...
  it('does not flag GLSL random( or numeric stagger', () => {
    const issues = lintVosConfig({
      ...
      createContent:
        '() => { const s = `float r = random(uv);`; return { objects: [] } }',
  ...
  it('flags wall-clock: Date.now, new Date, performance.now', () => {
    const rules = lintVosConfig({
      ...base,
      createContent:
        '() => { Date.now(); new Date(); performance.now(); return { objects: [] } }',
    }).map((i) => i.rule)
  ...
  it('warns on timers/rAF and network (not errors)', () => {
    const issues = lintVosConfig({
      ...base,
      onFrame:
        '() => { setTimeout(()=>{},1); requestAnimationFrame(()=>{}); fetch("/x") }',
    })
