...
export function generatePerLayerComposerSetup(config: VosConfig): string {
  ...
  // A function so a live content rebuild can run it again over the new groups.
  function __buildLayerComposers() {
    for (const group of renderGroups) {
      if (group.type !== '3d' || group.zIndex === 0 || group.composer) continue;
